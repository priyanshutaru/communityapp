import 'dart:convert';
import 'package:communityapp/screens/Events/event_comment_model.dart';
...
class event_Comment extends StatefulWidget {

  String? event_id,comment_count;
  event_Comment({required this.event_id,required this.comment_count});

  @override
  State<event_Comment> createState() => _event_CommentState();
}

class _event_CommentState extends State<event_Comment> {


  TextEditingController comment=TextEditingController();


  Future <List<Details>> comment_event() async {
    Map data={
      "event_id":"${widget.event_id.toString()}"
    };
    print("data"+data.toString());
    Uri url = Uri.parse("https://community.creditmywallet.in.net/api/get_event_comment");
    var body=jsonEncode(data);
    final response = await http.post(url, headers: {"Content-Type":
    "application/json"},body: body);
    print(response);
    if (response.statusCode == 200) {
      List data1 = json.decode(response.body)['details'];
      print(data1);
      return data1.map((job) => Details.fromJson(job)).toList();
    }
    else {
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text("${widget.comment_count}"+" total Comments",
                    style: TextStyle(
                        fontSize: 14,
                        fontWeight: FontWeight.w700,
                        color: Color(0xff000000)),
                                        ...
                                        children: [
                                          ...
                                          IconButton(
                                              onPressed: () async{
                                                ...
                                                var response=await dio.post('https://community.creditmywallet.in.net/api/delete_event_comment',
                                                    data: formData);
                                                setState(() {
                                                  print(response.toString()+"oiuytrewq  SDGHJMNXZ");
                                                });
          ...
          decoration: InputDecoration(border: OutlineInputBorder(borderRadius: BorderRadius.circular(30)),
            ...
            suffixIcon:  IconButton(
              onPressed: () async{
                ...
                print(formData.fields.toString());
                var response = await dio.post('https://community.creditmywallet.in.net/api/add_event_comment',
                    data: formData);
                var res=response.data;
                comment.clear();
                setState(() {
